...
RUN_DIR = "runs/amazon_ratings_phase2_full"
PLOT_DIR = "plots/amazon_ratings_phase2_full"
...
def main():
    ...
    # Alle Modelle und Hidden-Dimensionen nach Tiefe
    ...
    plt.title("Amazon-Ratings Phase 2 Full")
    ...
    depth_path = os.path.join(
        PLOT_DIR,
        "amazon_ratings_phase2_full_depth.png",
    )
    plt.savefig(depth_path, dpi=300)
    plt.close()

    # Beste Konfiguration je Modell
    best_rows = []
    ...
    plt.ylabel("Mean Best Test Accuracy")
    plt.title(
        "Amazon-Ratings Phase 2 Full – Best Configuration per Model"
    )
    ...
    best_path = os.path.join(
        PLOT_DIR,
        "amazon_ratings_phase2_full_best_bar.png",
    )
    plt.savefig(best_path, dpi=300)
    plt.close()

    # Baseline-Modelle aus dem Full-Block
    baseline_models = [
        "GCN",
        "GAT",
        "GraphSAGE",
    ...
    plt.xlabel("Number of Layers")
    plt.ylabel("Mean Best Test Accuracy")
    plt.title("Amazon-Ratings Phase 2 Full – Baselines Only")
    plt.xticks(sorted(df["num_layers"].unique()))
    plt.legend(fontsize=8)
    plt.tight_layout()
    ...
    baseline_path = os.path.join(
        PLOT_DIR,
        "amazon_ratings_phase2_full_baselines_only.png",
    )
