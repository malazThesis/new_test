...
PLOT_DIR = "plots/actor_comparisons"
os.makedirs(PLOT_DIR, exist_ok=True)

P1_BASE = pd.read_csv(
    "runs/actor_phase1_baselines/"
    "actor_phase1_baselines_grouped.csv"
)
P2_BASE = pd.read_csv(
    "runs/actor_phase2_baselines/"
    "actor_phase2_baselines_grouped.csv"
)
P1_FULL = pd.read_csv(
    "runs/actor_phase1_full/"
    "actor_phase1_full_grouped.csv"
)
P2_FULL = pd.read_csv(
    "runs/actor_phase2_full/"
    "actor_phase2_full_grouped.csv"
)
...
def plot_baselines_vs_full(
    ...
    plt.ylabel("Mean Best Test Accuracy")
    plt.title(
        f"Actor {phase_name}: Baselines vs Full"
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            PLOT_DIR,
            f"actor_{phase_name.lower().replace(' ', '')}"
...
plot_phase_comparison(
    P1_BASE,
    P2_BASE,
    "actor_baselines_phase1_vs_phase2.png",
    "Actor Baselines: Phase 1 vs Phase 2",
)

plot_phase_comparison(
    P1_FULL,
    P2_FULL,
    "actor_full_phase1_vs_phase2.png",
    "Actor Full: Phase 1 vs Phase 2",
)
