...
RUNS_DIR = Path("runs")
PLOT_DIR = Path("plots/all_realworld_selected4")
...
def filter_selected(frame: pd.DataFrame):
    result = frame[
        frame["dataset"].isin(SELECTED_DATASETS)
    ].copy()

    found = set(result["dataset"].unique())
    expected = set(SELECTED_DATASETS)

    missing = expected - found

    if missing:
        raise ValueError(
            f"Missing selected datasets: {sorted(missing)}"
        )
...
def plot_best_scores(comparison: pd.DataFrame):
    ...
    plt.title(
        "Best performance across selected real-world datasets"
    )

    plt.legend()

    save_current_figure(
        "all_realworld_best_scores_4datasets.png"
    )
...
def plot_depth_drop(comparison: pd.DataFrame):
    ...
    plt.title(
        ...
        "across selected real-world datasets"
    )

    plt.legend()

    save_current_figure(
        "all_realworld_depth_drop_4datasets.png"
    )
...
def plot_pairnorm_best_grid(pairnorm: pd.DataFrame):
    ...
    save_current_figure(
        "all_realworld_pairnorm_best_grid_4datasets.png"
    )
...
def plot_pairnorm_matched(matched: pd.DataFrame):
    ...
    save_current_figure(
        "all_realworld_pairnorm_matched_4datasets.png"
    )
...
def main():
    ...
    comparison = filter_selected(
        comparison
    )

    pairnorm = filter_selected(
        pairnorm
    )

    matched = filter_selected(
        matched
    )
    ...
    print(
        "Comparison datasets:",
        comparison["dataset"].nunique(),
    )

    print(
        "PairNorm datasets:",
        pairnorm["dataset"].nunique(),
    )

    print(
        "Matched datasets:",
        matched["dataset"].nunique(),
    )
    ...
    plot_pairnorm_best_grid(
        pairnorm
    )

    plot_pairnorm_matched(
        matched
    )
