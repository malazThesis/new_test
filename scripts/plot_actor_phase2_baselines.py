...
PLOT_DIR = "plots/actor_phase2_baselines"
...
def main():
    ...
    for model_name in sorted(df["model"].unique()):
        ...
        for hidden_channels in hidden_values:
            subset = df[
                (df["model"] == model_name)
    ...
    plt.title("Actor Phase 2 Baselines")
    ...
    depth_path = os.path.join(
        PLOT_DIR,
        "actor_phase2_baselines_depth.png",
    )
    ...
    plt.ylabel("Mean Best Test Accuracy")
    plt.title(
        "Actor Phase 2 Baselines – Best Configuration per Model"
    )
    plt.tight_layout()

    bar_path = os.path.join(
        PLOT_DIR,
        "actor_phase2_baselines_best_bar.png",
    )
