...
out_full_csv = os.path.join(run_dir, "realworld_phase2_summary.csv")
out_grouped_csv = os.path.join(run_dir, "realworld_phase2_grouped.csv")
out_best_grouped_csv = os.path.join(run_dir, "realworld_phase2_best_grouped_by_dataset.csv")
...
# Save full summary again for convenience
df = df.sort_values(
...
print("\n=== Best grouped config per dataset ===")
print(best_grouped.to_string(index=False))
