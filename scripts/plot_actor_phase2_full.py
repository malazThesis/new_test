...
RUN_DIR = "runs/actor_phase2_full"
PLOT_DIR = "plots/actor_phase2_full"
...
def main():
    ...
    plt.ylabel("Mean Best Test Accuracy")
    plt.title(
        "Actor Phase 2 Full – Best Configuration per Model"
    )
    ...
    best_path = os.path.join(
        PLOT_DIR,
        "actor_phase2_full_best_bar.png",
    )
    ...
    plt.xlabel("Number of Layers")
    plt.ylabel("Mean Best Test Accuracy")
    plt.title("Actor Phase 2 Full – Baselines Only")
    plt.xticks(sorted(df["num_layers"].unique()))
    plt.legend(fontsize=8)
    plt.tight_layout()
    ...
    baseline_path = os.path.join(
        PLOT_DIR,
        "actor_phase2_full_baselines_only.png",
    )
